<div align="center">
  <img src="./images/9router.png?1" alt="DXRouter Dashboard" width="800"/>
  
  # DXRouter - FREE AI Router & Token Saver
...
[🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Setup](#-setup-guide) • [🔀 Upstream](https://github.com/decolua/9router)
...
DXRouter is not on npm yet — `cli/package.json` declares the package name `dxrouter`, but
nothing has been published, so install from this checkout. Do not run
`npm install -g 9router`: that is upstream 9Router's package, and it gives you 9Router on a
different port.
...
**Development mode (this repository):**

This repository package is private (`dxrouter-app`), so source/Docker execution is the expected local development path.
...
```bash
DXR_ALLOW_NETWORK=1 PORT=20127 HOSTNAME=0.0.0.0 npm run start
```
...
> 🎬 **Made a video about DXRouter?** Submit a [Pull Request](https://github.com/iAmAjayTeli/dxrouter/pulls) adding your video to this section — we'll merge it!

---

## 🛠️ Supported CLI Tools

DXRouter works seamlessly with all major AI coding tools:
...
Headroom is optional and runs separately. DXRouter calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:

```
Client → DXRouter → Headroom /v1/compress → DXRouter → provider
```
...
If Headroom is down or returns an error, DXRouter fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → DXRouter translates → Provider receives native format
...
> DXRouter itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding DXRouter Costs & Billing

**DXRouter Billing Reality:**

✅ **DXRouter software = FREE forever** (open source, never charges)  
...
❌ **DXRouter never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, DXRouter just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **DXRouter**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through DXRouter.
...
<details>
<summary><b>💳 Will I be charged by DXRouter?</b></summary>

**No.** DXRouter is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, DXRouter just routes your requests
- ❌ **DXRouter itself** → **Never charges anything, ever**

DXRouter is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
DXRouter just routes your requests to them - there's no "catch" or future billing from DXRouter itself. They're truly free services, and DXRouter makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - DXRouter helps maximize their value through quota tracking
...
DXRouter's smart fallback prevents surprise charges:
...
**You're in control:** Set spending limits per provider in dashboard, and DXRouter respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. DXRouter tracks quota per model!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from DXRouter dashboard]
...
```json
{
  ...
  "anthropic_api_key": "your-dxrouter-api-key"
}
```
...
export OPENAI_API_KEY="your-dxrouter-api-key"
...
> **Note:** OpenClaw only works with local DXRouter. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
# Cloud sync stays off unless you point it somewhere. `https://9router.com` is upstream
# 9Router's hosted service — DXRouter does not operate it, and routing your traffic there
# is a deliberate choice rather than a default. Leave CLOUD_URL unset to stay local.
...
# Or use PM2
npm install -g pm2
pm2 start npm --name dxrouter -- start
pm2 save
pm2 startup
```
...
(`.github/workflows/docker-publish.yml`) fires only on a `v*` tag push and this repository
has no remote tags, so nothing has been published. Build from this checkout instead.

Do not pull `decolua/9router`. That is upstream 9Router's image: running it gives you a
working DXRouter and no DXRouter, on a different port.
...
```bash
docker logs -f dxrouter
docker restart dxrouter
docker stop dxrouter && docker rm dxrouter
```

Updating means rebuilding, not pulling: `git pull && docker compose up -d --build`.

**Data persistence:** `$HOME/.9router/db/data.sqlite` on host ↔ `/app/data/db/data.sqlite` in container.
The host directory stays `~/.9router` and the compose volume stays `9router-data` on
purpose — renaming either would orphan the database, credentials and usage history of
every existing install.

Full reference, including the Headroom sidecar and the GHCR publish flow: [DOCKER.md](DOCKER.md).
...
| `CLOUD_URL`                                          | _(none)_                                 | Server-side cloud sync endpoint base URL. Unset by default — `getCloudUrl()` in `src/lib/db/repos/settingsRepo.js` falls through to `""`, so cloud sync is off until you set it. `https://9router.com` is upstream 9Router's hosted service, which DXRouter does not operate. |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`                  | Backward-compatible/public base URL (prefer `BASE_URL` for server runtime)          |
| `NEXT_PUBLIC_CLOUD_URL`                              | _(none)_                                 | Backward-compatible/public cloud URL (prefer `CLOUD_URL` for server runtime). Also unset by default. |
...
- Since M0 there is **no default password**. The first start prints a generated credential once
  ...
  successful login. Lost it? `dxrouter` → Settings → “Reset Password (generates a new one)”.
...
- **GitHub**: [github.com/iAmAjayTeli/dxrouter](https://github.com/iAmAjayTeli/dxrouter)
- **Issues**: [github.com/iAmAjayTeli/dxrouter/issues](https://github.com/iAmAjayTeli/dxrouter/issues)
- **Upstream**: DXRouter is a fork of [decolua/9router](https://github.com/decolua/9router). `9router.com` is
  upstream's hosted service; DXRouter has no hosted service and runs locally.

---

## 👥 Contributors

Thanks to all contributors who helped make DXRouter better!
