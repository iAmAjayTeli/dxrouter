...
| `src/mitm/paths.js`, `src/lib/updater/updater.js` | Keep their own copy of the precedence, documented as such. | Both run outside the bundler (spawned scripts), so they cannot import `@/lib/dataDir`; they are the only two permitted duplicators and `tests/security/data-root.test.js` asserts they stay in step. |
