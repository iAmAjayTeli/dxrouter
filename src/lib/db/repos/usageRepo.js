...
export async function saveRequestUsage(entry) {
  try {
    ...
    //
    // Every call is one request. There used to be a lookup here that skipped the insert
    // when a row with the same millisecond timestamp, provider, model, connection, key and
    // token counts already existed. It was added (0d216689) to absorb a second write that
    // logUsage() made for every stream, and that write was removed at its source 41s later
    // (ec096d2a) — leaving a filter that silently discarded genuinely separate requests
    // finishing in the same millisecond with the same token counts: 100 concurrent
    // identical requests were recorded as 2 or 3, undercounting usage and cost.
    db.transaction(() => {
      db.run(
